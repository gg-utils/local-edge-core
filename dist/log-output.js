...
export function LocalEdgeCore_isTruthy(value) {
...
export function LocalEdgeCore_logInfo(message, options) {
    if (options.colorEnabled) {
        process.stdout.write(`${LOCAL_EDGE_CORE_ANSI_BOLD_CYAN}[local-edge]${LOCAL_EDGE_CORE_ANSI_RESET} ${message}\n`);
        return;
    }
    process.stdout.write(`[local-edge] ${message}\n`);
}
/** Prints `[local-edge] OK: <message>` to stdout. */
export function LocalEdgeCore_logSuccess(message, options) {
    if (options.colorEnabled) {
        process.stdout.write(`${LOCAL_EDGE_CORE_ANSI_BOLD_GREEN}[local-edge]${LOCAL_EDGE_CORE_ANSI_RESET} ${LOCAL_EDGE_CORE_ANSI_GREEN}OK:${LOCAL_EDGE_CORE_ANSI_RESET} ${message}\n`);
        return;
    }
    process.stdout.write(`[local-edge] OK: ${message}\n`);
}
/** Prints `[local-edge] WARNING: <message>` to stderr. */
export function LocalEdgeCore_logWarn(message, options) {
    if (options.colorEnabled) {
        process.stderr.write(`${LOCAL_EDGE_CORE_ANSI_BOLD_YELLOW}[local-edge]${LOCAL_EDGE_CORE_ANSI_RESET} ${LOCAL_EDGE_CORE_ANSI_YELLOW}WARNING:${LOCAL_EDGE_CORE_ANSI_RESET} ${message}\n`);
        return;
    }
    process.stderr.write(`[local-edge] WARNING: ${message}\n`);
}
/** Prints `[local-edge] ERROR: <message>` to stderr. */
export function LocalEdgeCore_logError(message, options) {
    if (options.colorEnabled) {
        process.stderr.write(`${LOCAL_EDGE_CORE_ANSI_BOLD_RED}[local-edge]${LOCAL_EDGE_CORE_ANSI_RESET} ${LOCAL_EDGE_CORE_ANSI_RED}ERROR:${LOCAL_EDGE_CORE_ANSI_RESET} ${message}\n`);
        return;
    }
    process.stderr.write(`[local-edge] ERROR: ${message}\n`);
}
/** Prints a section header, optionally using the formatter marker contract. */
export function LocalEdgeCore_logSection(title, options) {
    if (options.formatterActive) {

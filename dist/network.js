 ...
 */
export function LocalEdgeCore_isTcpPortListening(options) {
...
export function LocalEdgeCore_isPortListening(options) {
    ...
    if (host !== undefined && host.length > 0) {
        const result = spawnSync("lsof", ["-nP", `-iTCP@${host}:${String(port)}`, "-sTCP:LISTEN"], { stdio: ["ignore", "ignore", "ignore"], timeout: 5000 });
        return result.status === 0;
    }
    const result = spawnSync("lsof", ["-nP", `-iTCP:${String(port)}`, "-sTCP:LISTEN"], { stdio: ["ignore", "ignore", "ignore"], timeout: 5000 });
    return result.status === 0;
}
...
export function LocalEdgeCore_waitForPort(options) {
    ...
    while (Date.now() < deadline) {
        if (LocalEdgeCore_isPortListening({
            port: options.port,
            host: options.host,
        })) {
            return true;
...
export function LocalEdgeCore_loopbackAliasExists(ip) {
    if (ip === "127.0.0.1") {
        return true;
    }
...
export function LocalEdgeCore_terminateListenersOnPort(port) {
    const result = spawnSync("lsof", [`-tiTCP:${String(port)}`, "-sTCP:LISTEN"], { stdio: ["ignore", "pipe", "ignore"], timeout: 5000 });
    if (result.status !== 0 || result.stdout === null) {
        return;
    }

...
export function LocalEdgeCore_buildUrlMatrixLines(options) {
    ...
    for (const surface of options.surfaces) {
        ...
        lines.push(`[local-edge:${options.method}] app=${surface.surface} ${LocalEdgeCore_formatHttpsUrl({
            host,
            port: options.port,
            requestPath: surface.appPath,
        })}`);
        lines.push(`[local-edge:${options.method}] status=${surface.surface} ${LocalEdgeCore_formatHttpsUrl({
            host,
            port: options.port,
...
//# sourceMappingURL=url-matrix.js.map
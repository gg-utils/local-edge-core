...
export function LocalEdgeCore_normalizeSlug(value) {
    return value
        .trim()
        .toLowerCase()
        .replace(/[^a-z0-9-]+/g, "-")
...
export function LocalEdgeCore_parseRequiredPositiveInt(options) {
    ...
    if (!Number.isFinite(parsed) || parsed < 1) {
        throw new Error(`[local-edge:config] ${options.envName} must be a positive integer. ` +
...
export function LocalEdgeCore_parseOptionalBooleanFlag(options) {
    ...
    throw new Error(`[local-edge:config] ${options.envName} must be a boolean flag ` +
...
export function LocalEdgeCore_parseRequiredHostOrIp(options) {
    ...
    if (!normalized || normalized.length === 0) {
        throw new Error(`[local-edge:config] ${options.envName} must be a non-empty host/IP value.`);
    }
    return normalized;
}
/** Returns trimmed non-empty text, or null when the input is missing or only whitespace. */
export function LocalEdgeCore_parseOptionalTrimmedValue(rawValue) {
...
export function LocalEdgeCore_assertUniquePorts(entries) {
    const seenPorts = new Map();
    for (const entry of entries) {
        const existingName = seenPorts.get(entry.port);
        if (existingName) {
            throw new Error(`[local-edge:config] Port collision: ${entry.name} and ${existingName} ` +
                `both resolved to ${entry.port}.`);
        }
        seenPorts.set(entry.port, entry.name);
    }
}
//# sourceMappingURL=config-primitives.js.map
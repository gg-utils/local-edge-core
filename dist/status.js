...
export function LocalEdgeCore_buildDockerComposeServiceContainerIdArgs(options) {
    return [
        "compose",
        "-p",
        options.projectName,
...
export function LocalEdgeCore_buildDockerInspectRunningArgs(containerId) {
    return ["inspect", "-f", "{{.State.Running}}", containerId];
}
/** Parses the first non-empty container id emitted by `docker compose ps -q`. */
export function LocalEdgeCore_parseDockerComposeContainerId(stdout) {
...
export function LocalEdgeCore_parseDockerInspectRunningState(stdout) {
 ...
 */
export function LocalEdgeCore_probeDockerComposeServiceRunning(options) {
    try {
        const containerIdOutput = options.runDockerCommand(LocalEdgeCore_buildDockerComposeServiceContainerIdArgs({
            projectName: options.projectName,
            composePath: options.composePath,
            serviceName: options.serviceName,
        }));
        const containerId = LocalEdgeCore_parseDockerComposeContainerId(containerIdOutput);
        if (containerId === null) {
            return false;
        }
        const runningStateOutput = options.runDockerCommand(LocalEdgeCore_buildDockerInspectRunningArgs(containerId));
...
export function LocalEdgeCore_buildStatusReport(options) {
    const statusesByMethod = new Map(options.methodStatuses.map((status) => [status.method, status]));
    const allRunning = options.configuredMethods.every((method) => statusesByMethod.get(method)?.running === true);
    const selectedStatus = options.selectedMethod === "all"
        ...
        : {
            ...
            running: statusesByMethod.get(options.selectedMethod)?.running === true,
        };
    return {
        method: options.selectedMethod,
...
function LocalEdgeCore_requireMethodStatus(options) {
    const status = options.report.methodStatuses.find((candidate) => candidate.method === options.method);
    if (!status) {
        throw new Error(`[local-edge-core:status] Missing status for method '${options.method}'.`);
    }
    return status;
}
/** Renders the status report using the legacy local-edge text format. */
export function LocalEdgeCore_renderStatusText(report) {
    const lines = [];
    if (report.method === "all") {
        lines.push(`[local-edge:status] configuredMethods=${report.configuredMethods}`);
        for (const method of report.configuredMethods.split(",").filter(Boolean)) {
            ...
            lines.push(`[local-edge:status] ${method}  bind=${status.host}:${status.port} running=${String(status.running)}`);
        }
    }
    else {
        ...
        lines.push(`[local-edge:status] ${report.method}  bind=${status.host}:${status.port} running=${String(status.running)}`);
    }
...
export function LocalEdgeCore_renderStatusJson(report) {

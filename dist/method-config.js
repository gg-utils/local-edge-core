...
import { LocalEdgeCore_normalizeLegacyArtifactPath, LocalEdgeCore_normalizeMachineRootDir, } from "./paths.js";
import { LocalEdgeCore_buildDockerComposeDownCommand, LocalEdgeCore_buildDockerComposeUpDetachedCommand, } from "./docker-lifecycle.js";
/** Normalizes a local-edge route/config segment with the legacy `default` empty fallback. */
export function LocalEdgeCore_slugifySegment(rawValue) {
...
export function LocalEdgeCore_resolveDeveloperSlug(options) {
    const raw = options.configuredDeveloperSlug !== undefined
...
export function LocalEdgeCore_resolveEnvSlug(options) {
    return LocalEdgeCore_slugifySegment(options.configuredEnvSlug ?? options.defaultEnvSlug);
}
...
export function LocalEdgeCore_resolveNginxDockerProjectName(options) {
    ...
    return LocalEdgeCore_slugifySegment(`${options.projectNamePrefix}-${developerSlug}-${envSlug}`);
}
...
export function LocalEdgeCore_validateRequestedMethod(options) {
    ...
    if (LocalEdgeCore_isSupportedMethod({
        method: options.method,
        supportedMethods: options.supportedMethods,
    })) {
        return options.method;
    }
    throw new Error(`[${options.commandLabel}] Unsupported --method '${options.method}'.`);
}
...
export function LocalEdgeCore_resolveNginxDockerDefaultComposePath(options) {
    return path.join(options.generatedDir, "router", "nginx-docker", "docker-compose.yml");
}
...
export function LocalEdgeCore_resolveNginxDockerConfigPath(options) {
    return LocalEdgeCore_normalizeLegacyArtifactPath({
        ...
        machineRootDir: options.generatedDir,
        fallbackPath: path.join(options.generatedDir, "router", "nginx-docker", "nginx.conf"),
    });
}

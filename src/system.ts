...
export function LocalEdgeCore_isMacos(): boolean {
  return process.platform === "darwin";
}

/** Resolves a command path from `$PATH`, with a Docker Desktop fallback on macOS for `docker`. */
export function LocalEdgeCore_resolveCommandPath(
  commandName: string,
): string | null {
...
export function LocalEdgeCore_commandExists(commandName: string): boolean {
  return LocalEdgeCore_resolveCommandPath(commandName) !== null;
}

/** Returns true when stdin and stdout are both TTYs. */
export function LocalEdgeCore_hasInteractiveTerminal(): boolean {
  return Boolean(process.stdin.isTTY) && Boolean(process.stdout.isTTY);
}

/** Resolves an absolute canonical path. */
export function LocalEdgeCore_canonicalPath(rawPath: string): string {

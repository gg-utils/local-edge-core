...
export function LocalEdgeCore_renderNginxWebsocketProxyDirectives(): string {
...
export function LocalEdgeCore_renderNginxServerBlock(
  options: LocalEdgeCore_NginxServerBlockOptions,
): string {
...
function LocalEdgeCore_renderNginxCommentLines(
...
function LocalEdgeCore_renderNginxDockerResolverBlock(
  options: LocalEdgeCore_NginxDockerResolverOptions | null,
): string {
  if (!options) {
    return "";
  }
...
export function LocalEdgeCore_renderNginxHttpConfig(
  options: LocalEdgeCore_NginxHttpConfigOptions,
): string {
  const timeoutComment = LocalEdgeCore_renderNginxCommentLines(
    options.timeoutCommentLines,
  );
  const dockerResolverBlock = LocalEdgeCore_renderNginxDockerResolverBlock(
    options.dockerResolver,
  );
...
function LocalEdgeCore_renderDockerComposeLabel(label: string): string {
  return `      - "${label}"`;
}

/** Renders one Docker Compose port binding entry. */
function LocalEdgeCore_renderDockerComposePortBinding(
  binding: LocalEdgeCore_DockerComposePortBinding,
): string {
  return `      - "${binding.host}:${binding.hostPort}:${binding.containerPort}"`;
}

/** Renders one Docker Compose volume mount entry. */
function LocalEdgeCore_renderDockerComposeVolumeMount(
  mount: LocalEdgeCore_DockerComposeVolumeMount,
): string {
...
export function LocalEdgeCore_renderNginxDockerCompose(
  options: LocalEdgeCore_NginxDockerComposeOptions,
): string {

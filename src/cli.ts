...
export function LocalEdgeCore_parseCliArgs(
  args: readonly string[],
): LocalEdgeCore_CliParseResult {
  const result: LocalEdgeCore_CliParseResult = {
    dryRun: false,
    format: "text",
    manifestPath: null,
    help: false,
...
export function LocalEdgeCore_parseRenderCliArgs(options: {
  ...
  let mode: "execute" | "check" = "execute";
...
export function LocalEdgeCore_renderCliOutput(
  options: LocalEdgeCore_CliParseResult,
): string {
  if (options.help) {
    return [
      "local-edge core",
...
export function LocalEdgeCore_runCli(args: readonly string[]): number {
  try {

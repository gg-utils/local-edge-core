...
export type LocalEdgeCore_DoctorCliCommand =
  ...
  | { readonly command: "print-registry-line"; readonly registryPath: string }
  | {
  ...
  | { readonly command: "print-render-line"; readonly artifactPath: string }
  | { readonly command: "print-tls-wildcards"; readonly registryPath: string };

/** Generic object record used for loose legacy JSON summarization. */
type LocalEdgeCore_DoctorRecord = {
  readonly [key: string]: unknown;
};

/** Narrows unknown JSON values to a non-null object record for field reads. */
function LocalEdgeCore_doctorIsRecord(
  value: unknown,
): value is LocalEdgeCore_DoctorRecord {
 ...
 * registry payloads.
 */
export function LocalEdgeCore_doctorRealmValuesFromPayload(
  payload: unknown,
): unknown[] {
 ...
 */
export function LocalEdgeCore_doctorParseJsonText(jsonText: string): unknown {
...
export function LocalEdgeCore_doctorRenderUsage(): string {
 ...
 */
export function LocalEdgeCore_doctorParseArgv(
  argv: readonly string[],
): LocalEdgeCore_DoctorCliCommand {
...
export function LocalEdgeCore_doctorSummarizeRegistryPayload(
  payload: unknown,
): LocalEdgeCore_DoctorRegistrySummary {
...
export function LocalEdgeCore_doctorFormatRegistrySummaryLine(
  summary: LocalEdgeCore_DoctorRegistrySummary,
): string {
 ...
 */
export function LocalEdgeCore_doctorSummarizeRenderArtifactPayload(
  payload: unknown,
): LocalEdgeCore_DoctorRenderArtifactSummary {
...
export function LocalEdgeCore_doctorFormatRenderArtifactLine(
  summary: LocalEdgeCore_DoctorRenderArtifactSummary,
): string {
  const realms =
    summary.realmsJoined.length > 0 ? summary.realmsJoined : "none";
...
export function LocalEdgeCore_doctorCollectActiveRealmTlsWildcards(
  payload: unknown,
): string[] {
  const realmValues = LocalEdgeCore_doctorRealmValuesFromPayload(payload);
...
export function LocalEdgeCore_doctorFormatMissingWorkspaceRegistryLine(
  registryPath: string,
): string {
...
export function LocalEdgeCore_doctorVerifyWorkspaceRegistrationPayload(
  options: LocalEdgeCore_DoctorVerifyWorkspaceRegistrationPayloadOptions,
): LocalEdgeCore_DoctorVerifyWorkspaceRegistrationResult {
  const realmValues = LocalEdgeCore_doctorRealmValuesFromPayload(
    options.payload,
  );

  const matched = realmValues.some(
    (realm) =>
      LocalEdgeCore_doctorIsRecord(realm) &&
...
export function LocalEdgeCore_doctorVerifyWorkspaceRegistration(
  options: LocalEdgeCore_DoctorVerifyWorkspaceRegistrationOptions,
): LocalEdgeCore_DoctorVerifyWorkspaceRegistrationResult {
  if (!fs.existsSync(options.registryPath)) {
    return {
      ...
      exitCode: 1,
      stderrLine: LocalEdgeCore_doctorFormatMissingWorkspaceRegistryLine(

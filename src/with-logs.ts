...
export const LOCAL_EDGE_CORE_WITH_LOGS_SHUTDOWN_PREFIX = "[local-edge:shutdown] " as const;

/** Discriminated lifecycle events for typed single-line formatting. */
export type LocalEdgeCore_WithLogsLifecycleEvent =
  | { readonly kind: "checkout_kind_resolve_failed"; readonly projectRoot: string }
  | { readonly kind: "reclaim_session"; readonly projectRoot: string }
  | { readonly kind: "runtime_exited"; readonly exitCode: number }
  | { readonly kind: "shutdown_parent_message"; readonly message: string };
...
export type LocalEdgeCore_WithLogsParseResult =
  ...
  | { readonly command: "print-prelude"; readonly logFile: string }
  | { readonly command: "print-shutdown-prefix" }
  | { readonly command: "format-checkout-kind-error"; readonly projectRoot: string }
  | { readonly command: "format-reclaim-notice"; readonly projectRoot: string }
  | { readonly command: "format-runtime-exit"; readonly exitCode: number };
...
export function LocalEdgeCore_withLogsRenderUsage(): string {
...
export function LocalEdgeCore_withLogsFormatShutdownParentLine(message: string): string {
...
export function LocalEdgeCore_withLogsFormatCheckoutKindResolveFailed(
  projectRoot: string,
): string {
...
export function LocalEdgeCore_withLogsFormatReclaimNotice(projectRoot: string): string {
...
export function LocalEdgeCore_withLogsFormatRuntimeExit(exitCode: number): string {
  return LocalEdgeCore_withLogsFormatShutdownParentLine(
    `Runtime process exited with code ${String(exitCode)}.`,
  );
}

/** Returns true only for stdout EPIPE errors that can be ignored during force-kill cleanup. */
export function LocalEdgeCore_withLogsIsIgnorableStdoutError(error: unknown): boolean {
...
export function LocalEdgeCore_withLogsFormatLifecycleEvent(
  event: LocalEdgeCore_WithLogsLifecycleEvent,
): string {
  switch (event.kind) {
...
function LocalEdgeCore_withLogsExpectNoExtraArgs(
  commandLabel: string,
  rest: readonly string[],
): void {
  if (rest.length > 0) {
    throw new LocalEdgeCoreWithLogsCliError(
      `Unexpected arguments for ${commandLabel}: ${rest.join(" ")}`,
    );
  }
}
...
function LocalEdgeCore_withLogsParseSingleRequiredFlag(options: {
  ...
  readonly unknownTokenMessage: (token: string) => string;
}): string {
  let value = "";
...
function LocalEdgeCore_withLogsParseExitCodeFlag(rest: readonly string[]): number {
  const raw = LocalEdgeCore_withLogsParseSingleRequiredFlag({
    flag: "--exit-code",
    missingMessage: "format-runtime-exit requires --exit-code <integer>.",
    rest,
    unknownTokenMessage: (token: string) =>
      `Unknown argument for format-runtime-exit: ${token}`,
  });
...
export function LocalEdgeCore_withLogsParseArgv(
  argv: readonly string[],
): LocalEdgeCore_WithLogsParseResult {
  ...
  if (command === "format-checkout-kind-error") {
    return {
      ...
      projectRoot: LocalEdgeCore_withLogsParseSingleRequiredFlag({
        ...
        rest,
        unknownTokenMessage: (token: string) =>
          `Unknown argument for format-checkout-kind-error: ${token}`,
      }),
    };
  }
  ...
  if (command === "format-reclaim-notice") {
    return {
      ...
      projectRoot: LocalEdgeCore_withLogsParseSingleRequiredFlag({
        ...
        rest,
        unknownTokenMessage: (token: string) =>
          `Unknown argument for format-reclaim-notice: ${token}`,
      }),
    };
  }

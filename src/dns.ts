...
export type LocalEdgeCore_DnsAuditParseResult =
  | { readonly ok: true; readonly options: LocalEdgeCore_DnsAuditOptions }
  | { readonly ok: false; readonly message: string; readonly exitCode: 2 };
...
export function LocalEdgeCore_renderDnsmasqAddressRule(
  rule: LocalEdgeCore_SplitDnsAddressRule,
): string {
...
export function LocalEdgeCore_renderDnsmasqConfig(
  options: LocalEdgeCore_RenderDnsmasqConfigOptions,
): string {
  const lines = [
    `port=${options.port}`,
  ...
  if (options.defaultIpAddress && options.defaultIpAddress.length > 0) {
    ...
    if (!rootZone) {
      throw new Error(
        "[local-edge-core:dns] defaultIpAddress requires at least one address rule with rootZone.",
      );
    }
...
export function LocalEdgeCore_renderMacosResolverContent(
  options: LocalEdgeCore_RenderMacosResolverOptions,
): string {
  return [
...
export function LocalEdgeCore_parseMacosResolverContent(
  content: string,
): LocalEdgeCore_ParsedMacosResolver {
...
export function LocalEdgeCore_collectRequiredLoopbackAliases(
  options: LocalEdgeCore_RequiredLoopbackAliasesOptions,
): string[] {
...
export function LocalEdgeCore_createDnsAuditState(): LocalEdgeCore_DnsAuditState {
  return {
    checkTotal: 0,
    checkPassed: 0,
    checkFailed: 0,
...
export function LocalEdgeCore_recordDnsAuditPass(
  state: LocalEdgeCore_DnsAuditState,
): void {
  state.checkTotal += 1;
  state.checkPassed += 1;
}
...
export function LocalEdgeCore_formatDnsAuditPassLine(label: string): string {
...
export function LocalEdgeCore_formatDnsAuditSummaryLine(
  state: LocalEdgeCore_DnsAuditState,
): string {
  return `[local-edge:dns-audit] Summary checksPassed=${String(
    state.checkPassed,
  )}/${String(state.checkTotal)} checksFailed=${String(state.checkFailed)}`;
}

/** Renders the DNS audit artifact body without writing to disk. */
export function LocalEdgeCore_renderDnsAuditArtifact(
  options: LocalEdgeCore_RenderDnsAuditArtifactOptions,
): string {
  const status = options.state.checkFailed > 0 ? "failed" : "ok";
  const failureLines =
    options.state.failures.length === 0 ? "none" : options.state.failures.join("\n");

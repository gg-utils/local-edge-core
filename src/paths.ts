...
function LocalEdgeCore_collapseSlashes(input: string): string {
  let out = "";
 ...
 */
export function LocalEdgeCore_normalizeMachineRootDir(
 ...
 */
export function LocalEdgeCore_normalizeLegacyArtifactPath(
  options: LocalEdgeCore_NormalizeLegacyArtifactPathOptions,
): string {
  ...
  const normalizedMachineRoot = LocalEdgeCore_collapseSlashes(
    options.machineRootDir,
  );

  if (
    normalizedPath.endsWith("/generated/nginx-docker/main/docker-compose.yml")
  ) {
    return LocalEdgeCore_collapseSlashes(
      path.join(
        normalizedMachineRoot,
        "router",
        "nginx-docker",
        "docker-compose.yml",
      ),
  ...
  if (normalizedPath.endsWith("/generated/nginx-docker/main/nginx.conf")) {
    return LocalEdgeCore_collapseSlashes(
      path.join(normalizedMachineRoot, "router", "nginx-docker", "nginx.conf"),
    );
  }
  ...
  if (certsIndex >= 0) {
    ...
    return LocalEdgeCore_collapseSlashes(
      path.join(normalizedMachineRoot, "certs", tail),
    );
  }
...
export function LocalEdgeCore_collectActiveEnvSourceFiles(
  options: LocalEdgeCore_CollectActiveEnvSourceFilesOptions,
): string[] {
